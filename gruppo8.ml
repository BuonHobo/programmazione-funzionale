...
let rec max_common_subtree t1 t2 =
  match (t1, t2) with
  | Empty, _ | _, Empty -> Tr (0, Empty, Empty)
  | Tr (a, al, ar), Tr (b, bl, br) ->
      if a <> b then Tr (0, Empty, Empty)
...
let rec path p = function
  | Empty -> []
  | Tr (x, l, r) ->
      if p x then failwith "not found"
...
let rec percorsi_foglia x = function
  ...
  | Tr (y, Empty, Empty) -> if x = y then [ [ x ] ] else failwith "Not_found"
  | Tr (y, l, r) ->
  ...
  | Tr (y, l, r) -> (
      let rec aux col_prec = function
        ...
        | Tr (z, Empty, Empty) ->
            ...
            if z_col <> col_prec && x = z then [ z ]
            else failwith "Invalid_path"
        | Tr (z, l, r) ->
            let z_col = colore z colass in
            if z_col = col_prec then failwith "Invalid_path"
...
let rec get_rightmost = function
  | Empty -> failwith "empty tree"
  | Tr (x, l, r) -> ( try get_rightmost r with _ -> Tr (x, l, r))
