...
let conta_oggetti sponda =
    let rec aux miss cann barca= function
        []-> miss,cann,barca
        | Miss::rest -> aux (miss+1) cann barca rest
        | Cann::rest -> aux miss (cann+1) barca rest
        | Barca::rest -> aux miss cann (barca+1) rest
    in aux 0 0 0 sponda

let safe_sponda sponda =
    let miss,cann,_=conta_oggetti sponda
    in miss>=cann

let safe (sx,dx)=
    safe_sponda sx && safe_sponda dx

let rec crea_lista_n n obj=
    if n=0 then []
    else obj::crea_lista_n (n-1) obj

let crea_sponda (miss,cann,barca)=
    if miss<0 || cann<0 || barca<0 || (miss>0 && cann>miss)
    then failwith "Sponda non valida"
    else (crea_lista_n barca Barca) @ (crea_lista_n miss Miss) @ (crea_lista_n cann Cann)
...
let from_sit sit =
    List.filter_map (fun act -> try Some(applica act sit) with _-> None) actions
...
let nodo_visitato nodo visitati=
    List.exists (stessa_situazione nodo) visitati

let goal sit=
    conta_oggetti (fst initial) = conta_oggetti (snd sit)
...
let rec mosse n=
    if n=1 then from_sit initial
    else List.concat_map (from_sit) (mosse(n-1))

let from_sit2 sit=
    List.filter_map (fun act -> try Some(act,applica act sit) with _-> None) actions
    ...
    and aux_prossimi nodi visitati=
        match nodi with 
            ...
            |a::rest-> 
                try aux_nodo a visitati
                with _->aux_prossimi rest visitati
    in aux_prossimi (from_sit2 initial) [initial]