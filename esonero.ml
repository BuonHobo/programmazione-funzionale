...
let bin k tr=
    let rec aux peso cammino= function
        Empty->failwith "bin"
        | Tr(x,_,_) when (x+peso)>k-> failwith "bin"
        | Tr(x,Empty,Empty)-> (List.rev (x::cammino),x,peso+x)
        | Tr(x,lc,rc)->
...
let ntr k tr=
    let rec aux_nodo peso cammino (Tr(x,tlst))=
        if (peso+x) > k then failwith "ntr"
        else match tlst with
            []-> (List.rev (x::cammino),x,x+peso)
            |_-> aux_lista (peso+x) (x::cammino) tlst
    and aux_lista peso cammino = function
        []-> failwith "ntr"
        | a::rest->
            try aux_nodo peso cammino a 
            with _-> aux_lista peso cammino rest
    in aux_nodo 0 [] tr
