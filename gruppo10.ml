...
let vicini grafo nodo=
    List.filter_map (function
                        f,t when f=nodo -> Some(t)
                        | f,t when t=nodo -> Some(f)
                        | _,_ -> None
                    ) grafo
...
let test_connessi grafo n m =
    ...
    and aux_prossimi nodi visitati=
        match nodi with
            ...
            | a::rest-> 
                aux_nodo a visitati 
                || aux_prossimi rest visitati
    in aux_prossimi (prossimi grafo n) [n]
...
let ciclo grafo n =
    ...
    and aux_prossimi nodi visitati=
        match nodi with
            []->failwith "Not found"
            | a::rest -> 
                try aux_nodo a visitati
                with _ -> aux_prossimi rest visitati
    in aux_prossimi (prossimi grafo n) [n]
...
let visita_vicini grafo n=
    ...
    and aux_vicini nodi visitati=
        match nodi with
            []->visitati
            | a::rest -> aux_vicini rest (aux_nodo a visitati)
    in aux_nodo n []

let grafo_connesso (nodi,archi)=
    match nodi with
        []->true
        |_ ->List.for_all (fun nodo->List.mem nodo (visita_vicini archi (List.hd nodi))) nodi 

let rec rimuovi n = function
    []->[]
    |a::rest when a=n-> rest
    |a::rest-> a::rimuovi n rest
...
let cammino grafo lst n m =
    ...
    and aux_prossimi nodi ammessi=
        match nodi with
            ...
            |a::rest ->try aux_nodo a ammessi with _-> aux_prossimi rest ammessi
    in aux_nodo n lst

let hamiltoniano grafo=
    let lista_nodi=fst grafo in
    if lista_nodi=[] then failwith "empty" else

    let rec aux_nodo nodo non_visitati=
        if not (List.mem nodo non_visitati)
        then failwith "already visited"
        else 
            if [nodo]=non_visitati
            then [nodo]
            else nodo::aux_prossimi (prossimi (snd grafo) nodo) (rimuovi nodo non_visitati)
    and aux_prossimi nodi non_visitati=
        match nodi with
            []->failwith "not found"
            | a::rest -> try aux_nodo a non_visitati with _ -> aux_prossimi rest non_visitati

    in aux_nodo (List.hd lista_nodi) lista_nodi

type col = Rosso | Giallo | Verde | Blu
type 'a col_assoc = (col * 'a list) list

let colore assoc n=
    fst ( List.find (function _,lst-> List.mem n lst) assoc)
...
let colori_alterni grafo assoc start goal=
    ...
    and aux_prossimi nodi visitati ultimo=
        match nodi with 
            []->failwith"not found"
            |a::rest-> try aux_nodo a visitati ultimo with _-> aux_prossimi rest visitati ultimo
    in aux_prossimi (prossimi start) [start] (colore start)

let rec connessi_in_glist grafi b c=
    b<>c 
    && match grafi with
        []-> false
        | grafo::rest -> test_connessi grafo b c || connessi_in_glist rest b c

let cammino_con_nodi grafo n lst=
    let rec aux_nodo nodo visitati da_visitare=
        if List.mem nodo visitati then failwith "già visitato"
        else 
            if [nodo]=da_visitare
            then List.rev (nodo::visitati)
            else aux_prossimi (prossimi grafo nodo) (nodo::visitati) (rimuovi nodo da_visitare)
    and aux_prossimi nodi visitati da_visitare=
        match nodi with
            []->failwith "non trovato"
            |a::rest -> try aux_nodo a visitati da_visitare with _-> aux_prossimi rest visitati da_visitare
    in aux_nodo n [] lst

let is_primo n=
    let rec aux k=
        k>n/2 || (n mod k <> 0 && aux (k+1))
    in aux 2
...
let is_negazione f1 f2=
    f1 = Not(f2) || f2 = Not(f1)

let rec contiene_contraddizione f = function
    []->false
    | a::rest -> is_negazione a f || contiene_contraddizione f rest
...
let path_n_p grafo p n start=
    let rec aux_nodo nodo visitati k=
        ...
        then failwith "non valido"
            ...
            if k=1
            then List.rev (nodo::visitati)
            else aux_prossimi (prossimi grafo nodo) (nodo::visitati) (if p nodo then (k-1) else k)
    and aux_prossimi nodi visitati k=
        match nodi with
            []-> failwith "non trovato"
            | a::rest -> try aux_nodo a visitati k with _-> aux_prossimi rest visitati k
    in aux_nodo start [] n