...
let rec giraDopoChiusa = function
  | Chiusa :: a :: rest ->
      Chiusa :: gira a :: rest
  | a :: rest ->
      a :: giraDopoChiusa rest
  | _ ->
      failwith "giraDopoChiusa: impossibile"

let successori cassa =
  (try [ giraPrima cassa ] with _ -> [])
  @ try [ giraDopoChiusa cassa ] with _ -> []

let rec nodi n =
  let prec = if n = 1 then [ [] ] else nodi (n - 1) in
  List.map (List.cons Aperta) prec @ List.map (List.cons Chiusa) prec

let archi n = List.map (fun cassa -> (cassa, successori cassa)) (nodi n)
let start n = List.init n (fun _ -> Chiusa)
let aperta cassa = List.for_all (( = ) Aperta) cassa
