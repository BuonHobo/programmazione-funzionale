...
let rec subexpr e1 e2 =
  ...
  | MultiDiv (e1a, e1b) ->
      subexpr e1a e2 || subexpr e1b e2
  | MultiSum lst ->
      List.exists (fun e -> subexpr e e2) lst
  | MultiMult lst ->
      List.exists (fun e -> subexpr e e2) lst
  | _ ->
      false
...
let rec subst e1 x e2 =
  ...
  | MultiVar c when c = x ->
      e2
  | MultiDiff (e1a, e1b) ->
      MultiDiff (subst e1a x e2, subst e1b x e2)
  | MultiDiv (e1a, e1b) ->
      MultiDiv (subst e1a x e2, subst e1b x e2)
  | MultiSum lst ->
      MultiSum (List.map (fun e -> subst e x e2) lst)
  | MultiMult lst ->
      MultiMult (List.map (fun e -> subst e x e2) lst)
  | _ ->
      e1
...
let rec inorder (Tr (v, lst)) =
  ...
  | [] ->
      [ v ]
  | a :: rest ->
      inorder a @ (v :: List.concat_map inorder rest)
...
let rec foglie_in_lista lst (Tr (v, trees)) =
  ...
  | [] ->
      List.mem v lst
  | _ ->
      List.for_all (foglie_in_lista lst) trees
...
let rec num_di_foglie (Tr (_, lst)) =
  ...
  | [] ->
      1
  | a :: rest ->
      List.fold_left ( + ) 0 (List.map num_di_foglie lst)
...
let rec listaGuida lst (Tr (x, trees)) =
  ...
  | [] ->
      Tr (x, trees)
  | a :: rest ->
      listaGuida rest (List.nth trees a)
...
let rec cerca_foglia guida (Tr (x, lst)) =
  ...
  | [] ->
      x
  | _ ->
      cerca_foglia guida (List.nth lst (List.assoc x guida - 1))
...
let rec contiene_duplicati = function
  | [] ->
      false
  | a :: rest ->
      List.mem a rest || contiene_duplicati rest
...
let rec tutti_cammini_foglie (Tr (x, trees)) =
  ...
  | [] ->
      [ [ x ] ]
  | _ ->
      List.map (List.cons x) (List.concat_map tutti_cammini_foglie trees)
...
let ramo_di_primi tr =
  let rec aux1 (Tr (x, trees)) =
    ...
    else match trees with [] -> x | _ -> aux2 trees
  and aux2 = function
    | [] ->
        failwith "Not_found"
    | a :: rest -> (
